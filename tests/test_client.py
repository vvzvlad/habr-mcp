...
@respx.mock
async def test_get_article(anon_settings, article_payload):
...
@respx.mock
async def test_empty_2xx_body_is_success(author_settings):
    # A successful write may return 204/empty body; that must not raise.
    respx.delete(f"{BASE_URL}articles/drafts/100/posts").mock(
        return_value=httpx.Response(204)
    )
    client = HabrClient(author_settings)
    try:
        result = await client.delete_draft(100)
    finally:
        await client.aclose()
    assert result == {}
